...
// TransContext 事务的上下文，描述了数据事务，所有在该事务内执行的数据库操作都需要被提交或者回滚，保持原子性。在 godbx 里要想执行数据库操作必须要确定TransContext，
// 它数据操作的起点，一旦这个事务确定，对应的数据库连接就确定，底层物理事务就确定，同时它内部维护一个状态，用于记录事务的创建、提交和回滚, TransContext最终需要被调用
